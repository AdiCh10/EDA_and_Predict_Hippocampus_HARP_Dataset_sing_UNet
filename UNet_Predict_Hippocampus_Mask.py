...
#     # Save predicted mask as a temporary NIfTI file and return its file path for download
#     tmp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".nii")
#     nib.save(nib.Nifti1Image(mask, affine), tmp_file.name)
#     tmp_file.close()
#     return tmp_file.name
def save_mask(mask, affine):
    # Create temporary filename (not open file handle)
    tmp_dir = tempfile.gettempdir()
    tmp_path = os.path.join(tmp_dir, "predicted_mask.nii")

    # Ensure correct datatype for medical mask
...
if uploaded_file is not None:
    ...
    # Load trained model
    model_path = st.text_input("Path to trained U-Net Model", value=r"C:\Users\chaudadi\Projects\unet2d.pth")
